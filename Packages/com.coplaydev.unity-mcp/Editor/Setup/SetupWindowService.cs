...
namespace MCPForUnity.Editor.Setup
{
    ...
    public static class SetupWindowService
    {
        ...
        // Throttle dependency checks to avoid spawning processes on every domain reload.
        // Users without Python/uv would trigger CheckAllDependencies() on every script recompile otherwise.
        private const string DepCheckTimeKey = "MCPForUnity.LastDepCheckTime";
        private const double DepCheckCooldownSeconds = 300.0; // 5 minutes
        ...
        private static void CheckSetupNeeded()
        {
            ...
            try
            {
                ...
                {
                    // Permanently resolved — lock for this session.
                ...
                // Check dependencies — if Python/uv are not installed,
                // the user likely doesn't need MCP. Skip the popup silently.
                // Throttle: only re-check after cooldown to avoid spawning processes on every domain reload.
                double lastCheckTime = SessionState.GetFloat(DepCheckTimeKey, 0f);
                double now = EditorApplication.timeSinceStartup;
                if (lastCheckTime > 0 && (now - lastCheckTime) < DepCheckCooldownSeconds)
                {
                    return;
                }
                SessionState.SetFloat(DepCheckTimeKey, (float)now);

                var dependencyResult = DependencyManager.CheckAllDependencies();
                if (!dependencyResult.IsSystemReady)
                {
                    McpLog.Info(
                        "Setup Window skipped - dependencies not available (Python/uv not installed). " +
                        "Use Window > MCP for Unity > Local Setup Window to set up manually.",
                        always: false);
                    return;
                }

                // Dependencies are ready and setup not yet done — show window and lock session.
                SessionState.SetBool(SessionCheckedKey, true);
