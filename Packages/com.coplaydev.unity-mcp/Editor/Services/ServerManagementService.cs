...
namespace MCPForUnity.Editor.Services
{
    ...
    public class ServerManagementService : IServerManagementService
    {
        ...
        {
            // Check if Python/uv dependencies are available before attempting to start.
            // If not, show the Setup Window to guide installation instead of a generic error.
            try
            {
                var depResult = DependencyManager.CheckAllDependencies();
                if (!depResult.IsSystemReady)
                {
                    if (!quiet)
                    {
                        SetupWindowService.ShowSetupWindow(depResult);
                    }
                    return false;
                }
            }
            ...
            {
                McpLog.Warn($"Dependency check failed: {ex.Message}");
                if (!quiet)
                {
