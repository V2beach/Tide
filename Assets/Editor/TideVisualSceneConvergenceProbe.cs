...
public static class TideVisualSceneConvergenceProbe
{
    ...
    public static void RunFromCommandLine()
    {
        ...
        RunProbe("船骸潮窗拆卸", controller => controller.RunEditorWreckDismantleTideWindowProbe(), failures);
        RunProbe("首件材料兑现", controller => controller.RunEditorArrivalSalvagePayoffProbe(), failures);
        RunProbe("相邻高潮机会", controller => controller.RunEditorMixedSemidiurnalOpportunityProbe(), failures);
        RunProbe("可见浪物理同源", controller => controller.RunEditorLocalWaveEventFieldProbe(), failures);
        RunProbe("短航读浪反馈", controller => controller.RunEditorSailingWaveHandlingFeedbackProbe(), failures);
        ...
        RunProbe("潮尺预报", controller => controller.RunEditorTideForecastAutonomyProbe(), failures);
        RunProbe("网深实体相遇", controller => controller.RunEditorLiveNetControlAndLoadPhysicsProbe(), failures);
        RunProbe("自然潮离桩窗口", controller => controller.RunEditorNetExcursionWindowProbe(), failures);
        RunProbe("首潮岔流选择", controller => controller.RunEditorFirstTideRouteChoiceProbe(), failures);
        RunProbe("退潮漂积", controller => controller.RunEditorWrackLineLifecycleProbe(), failures);
        ...
        Debug.Log(
            "TIDE_VISUAL_SCENE_PROBE PASS | 开场/首日自主性/船骸潮窗拆卸/首件材料兑现/相邻高潮机会/可见浪物理/短航读浪/短航潮位/首航潮窗决策/潮尺预报/网深相遇/离桩窗口/岔流选择/退潮漂积/暴潮取舍/物资守恒/休息旁路/尺度/船上人物/行走/登船/三段梯/可走面/显式输入");
    }
