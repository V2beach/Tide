...
- 运行编排/表现：`TideMooringRopeController`；独立拥有输入占用、当前绳状态、收绳意图、断绳/靠稳一次性结果和绳形。Loose 时同一批线段落在码头形成待用绳圈，Swinging 后转到人物手上，Attached/Secured 后连接船艉；不能为提示另建第二根绳
...
- 部件维修性能：`TideBoatConditionPerformanceModel`；船壳只拥有漏率/船速上限，船帆只拥有风力效率/收放速度，舱底只拥有压舱速度/舀水效率/舀水时动量损失。`boatReadiness` 只保留为远航范围，不得重新进入即时操控公式
- 可见浪处理：`TideSailingWaveHandlingModel`；只有 `TideOceanSample.LocalWaveContact01` 证明浪头实际接触船体时才评估。收帆、用 Q/E 抵消浪坡和降低船水相对速度会保留动量、减少进水；主控制器只播放船艏水花/短音并移动座舱人物，不复制公式
...
- 返航窗口：`CanReturnFromSailing` 读取码头的水平接近和当前位置 `TideOceanSample.SurfaceY`；船在高潮或浪头上仍可按画面真实位置靠岸，不再比较固定 `sailingHomeY`
